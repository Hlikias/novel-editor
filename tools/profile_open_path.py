# -*- coding: utf-8 -*-
"""打开章节路径逐项计时：定位 1000 章项目下打开一章的耗时分布。"""
import os
import sys
import tempfile
import time
...
d = tempfile.mkdtemp(prefix="profopen_")
...
print("\n--- open_chapter 拆解（第 1 章）---")
t("get_chapter", lambda: st.get_chapter(1))
t("_new_editor(创建控件)", lambda: win._new_editor(1))
ed = win._new_editor(1)
ch = st.get_chapter(1)
t("set_content(HTML 渲染)", lambda: ed.set_content(ch.content))
t("_lines_with_bookmarks", lambda: win._lines_with_bookmarks(1))
t("_update_status", lambda: win._update_status())
t("_refresh_snap", lambda: win._refresh_snap())
t("_record_open_tabs", lambda: win._record_open_tabs())

print("\n--- chapter_snap_data 内部 ---")
t("chapter_snap_data 整体", lambda: chapter_snap_data(st, 1, "第 1 章"))
t("  list_chapter_cards", lambda: st.list_chapter_cards())
t("  list_foreshadows", lambda: st.list_foreshadows())
t("  list_storyline_nodes", lambda: st.list_storyline_nodes())
t("  setting_terms", lambda: st.setting_terms())
from app.chapter_snap import _strip_html
t("  _strip_html(4000字HTML)", lambda: _strip_html(ch.content))

print("\n--- 完整 open_chapter（非首次，全新窗口）---")
win2 = MainWindow()
win2._set_project(st)
t("open_chapter(1) 完整", lambda: win2.open_chapter(1))
win2.close()
win.close()
st.close()
import shutil
