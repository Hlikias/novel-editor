# -*- coding: utf-8 -*-
"""保存后 processEvents 全流程分解：定位压力测试中『保存一章』数秒耗时的真实来源。"""
import os
import sys
import tempfile
import time
...
d = tempfile.mkdtemp(prefix="profsave_")
...
print("\n--- 模拟压力测试的『编辑+保存+processEvents』全流程 ---")
t("ed.toPlainText()", lambda: ed.toPlainText())
t("ed.setPlainText(追加)", lambda: ed.setPlainText(ed.toPlainText() + "　　（测试）"))
t("save_current_chapter", lambda: win.save_current_chapter())
t("_status_timer 触发 _update_status", lambda: win._update_status())
t("_preview_timer 触发 _update_preview", lambda: win._update_preview())
t("_current_editor_html", lambda: win._current_editor_html())
t("preview_view.update_html", lambda: win.preview_view.update_html(win._current_editor_html()))
t("_tips_timer 触发 _update_tips", lambda: win._update_tips())
t("_do_aux_refresh", lambda: win._do_aux_refresh())
t("processEvents(全量)", lambda: app.processEvents())

print("\n--- 再跑一次 processEvents 看是否有累积 ---")
t("processEvents(第二次)", lambda: app.processEvents())
win.close()
st.close()
import shutil
